...
function createBooking($userId, $chargingPointId, $bookingDatetime) {
    error_log("🔧 createBooking() called with userId=$userId, chargingPointId=$chargingPointId, datetime=$bookingDatetime");

    // Controllo disponibilità
    if (!isChargingPointAvailable($chargingPointId, $bookingDatetime)) {
        error_log("❌ Charging point $chargingPointId is not available at $bookingDatetime");
        return false;
    }
    ...
    if ($bookingId) {
        ...
        // Aggiorna stato punto di ricarica
        $updated = update('Charging_Points',
            ['charging_point_state' => 'reserved'],
            'charging_point_id = ?',
            [$chargingPointId]
        );
        error_log("🔄 Charging point state updated: " . json_encode($updated));
